...
def count_csv_rows(file_path):
    ...
    try:
        ...
        print(f"读取文件时出错: {e}")
        return None
def counttime(datacode):# 计算上一次运行和几天的时间差值
    current_dir = os.path.dirname(__file__)
    df = pd.read_csv(os.path.join(current_dir, 'output', 'lastruntime',f'{datacode}flag.csv'), header=None)
...
def mainx(seq_length):
    ...
    runsumtime = EPOCHS*len(datalist)

    '''开始训练循环'''
    for datacode in datalist:
        '''根据上次训练距离现在有多久计算出EPOCH'''
        try:#
            EPOCHS = counttime(datacode)
        ...
        '''数据文件夹'''
        csv_dir = os.path.join(current_dir, 'data','rawdata',  f'{datacode}.csv')
        ...
        try:
            ...
            print(f'{datacode}数据异常')
            # signal_emitter.text_print.emit(abinfo_box,f'{stockcode}数据异常')
            continue
        joblib.dump(scaler, os.path.join(current_dir, 'output', 'pkl',f'{datacode}.pkl'))  # 保存归一化器
        # 处理测试数据（使用训练集的归一化参数）
        ...
        if os.path.exists(os.path.join(current_dir, 'output', 'model', f'{datacode}.pth')):# 读取训练过的模型
        # 加载保存的 checkpoint
            # signal_emitter.text_print.emit(info_box,f'{stockcode}读取曾训练模型')
            checkpoint = torch.load(os.path.join(current_dir, 'output', 'model', f'{datacode}.pth'), weights_only=True)
            ...
            if (epoch+1) % 10 == 0:
                ...
                with torch.no_grad():
                    try:
                        ...
                        print(f'{datacode}的数据可能有问题，数据移动回失败：{e}')
                        # signal_emitter.text_print.emit(abinfo_box,f'{stockcode}的数据可能有问题，数据移动回失败')
                        tryflag2 == 1
                        break
                    test_loss = criterion(test_preds, test_y.cpu())
                test_losses.append(test_loss.item())
                print(f'{datacode}Epoch [{epoch+1}/{EPOCHS}], Train Loss: {loss.item():.4f}, Test Loss: {test_loss.item():.4f}, rate{runtime/runsumtime:.4f}')
        ...
        plt.legend()
        plt.savefig(os.path.join(current_dir, 'output', 'loss', f'loss_{datacode}.png'))
        plt.close()
        ...
        if tryflag == 0 and tryflag2 == 0:
            ...
            plt.legend()
            plt.savefig(os.path.join(current_dir, 'output', 'predict', f'{datacode}.png'))
            # plt.show()
            plt.close()

            # 保存模型
            torch.save(model.state_dict(), os.path.join(current_dir, 'output', 'model', f'{datacode}.pth'))

            '''计算预测评估指标'''
            outputlevelloss = np.mean(np.abs(test_true - test_pred))
            outputlevellist.append(outputlevelloss)
            costture.append(datacode)
            # print(f'模型效果评估（MAE，越小越好）: {outputlevelloss}')
            flagfilename = os.path.join(current_dir, 'output','lastruntime', f'{datacode}flag.csv')# 记录模型上次被运行时什么时候
            runtime = runtime+1
